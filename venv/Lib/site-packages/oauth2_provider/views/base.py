...
from ..models import get_access_token_model, get_application_model, AccessToken, RefreshToken
    ...
    * Client credentials
    ...
    def post(self, request, *args, **kwargs):
        if request.POST['grant_type'] != 'refresh_token':
            ...
            try:
                AccessToken.objects.filter(user=HealthGuardUser.objects.filter(email=email)[0]).delete()
                RefreshToken.objects.filter(user=HealthGuardUser.objects.filter(email=email)[0]).delete()
